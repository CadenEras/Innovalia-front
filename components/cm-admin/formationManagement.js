...
function FormationManagement() {
	...
	useEffect(() => {
		fetch('http://51.77.213.191:8000/api/formations')
	...
	const createFormation = (event) => {
		...
		fetch('http://51.77.213.191:8000/api/formations', {
			...
			.then(response => response.json())
			.then(data => setFormations(prevFormations => [...prevFormations, data]));
	};

	const updateFormation = (id, updatedFormation) => {
		fetch(`http://51.77.213.191:8000/api/formations/${id}`, {
			method: 'PUT',
			headers: {
				'Content-Type': 'application/json',
			},
	...
	return (
		<div>
			...
			<form onSubmit={createFormation}>
				...
				<input type="text" placeholder="Type" onChange={e => setForm({ ...form, type: e.target.value })} />
			...
			{formations.map(formation => (
				<div key={formation.id}>
					...
					<button onClick={() => updateFormation(formation.id, form)}>Update</button>
