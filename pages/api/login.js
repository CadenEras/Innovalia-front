...
export default async function handler(req, res) {
	try {
		...
		if (!body) {
			return res.status(400).json({ message: 'Bad Request error: The request is invalid.' })
		}
		...
		const response = await axios.post('http://51.77.213.191:8000/api/auth/login', body, { headers });
